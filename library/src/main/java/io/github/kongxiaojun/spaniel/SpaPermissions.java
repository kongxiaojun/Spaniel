...
public class SpaPermissions {
    ...
    private static final String TAG = "SpaPermissions";
    ...
    private SpaPermissionCompatDelegate mPermissionCompatDelegate = new SpaPermissionCompatDelegate();

    private static class SpaPermissionsHolder {
        private static final SpaPermissions INSTANCE = new SpaPermissions();
    }

    private SpaPermissions() {
        initializePermissionsMap();
    }

    public static SpaPermissions getInstance() {
    ...
    /**
     * This method is the latest method to apply for permission. It only needs to receive the result in PermissionsResultAction. It is not necessary to receive the result in onRequestPermissionsResult method in Activity / Fragment.
     * Step：
    ...
    public synchronized void requestPermissions(@Nullable FragmentActivity fragmentActivity,
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            ...
            if (permList.isEmpty()) {
                ...
                if (!mPermissionCompatDelegate.requestPermissions(fragmentActivity, permsToRequest, Constant.PERMISSIONS_REQUEST_CODE)) {
    ...
    private void requestPermissionsFromFragment(FragmentActivity fragmentActivity, String[] permissions) {
        SpaPermissionsFragment permissionsFragment = getPermissionsFragment(fragmentActivity.getSupportFragmentManager());
        if (permissionsFragment.isAdded()) {
            permissionsFragment.requestPermissions(permissions, Constant.PERMISSIONS_REQUEST_CODE);
        } else {
    ...
    private SpaPermissionsFragment getPermissionsFragment(@NonNull final FragmentManager fragmentManager) {
        SpaPermissionsFragment permissionsFragment = findPermissionsFragment(fragmentManager);
        boolean isNewInstance = permissionsFragment == null;
        if (isNewInstance) {
            permissionsFragment = new SpaPermissionsFragment();
            fragmentManager
                    .beginTransaction()
    ...
    public synchronized void notifyPermissionsChange(Context context, @NonNull String[] permissions, @NonNull int[] results) {
        ...
        while (iterator.hasNext()) {
            ...
            for (int n = 0; n < size; n++) {
                if (action != null) {
                    action.onRequestPermissionsResult(Constant.PERMISSIONS_REQUEST_CODE, permissions, results);
                    if (!handled) {
